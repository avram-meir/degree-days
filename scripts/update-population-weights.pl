...
# --- Set weighting region definitions ---
...
my %us_states   = map { $_ => 1 } @us_states;
my @conus_states   = qw(AL AZ AR CA CO CT DE FL GA ID IL IN IA KS KY LA ME MD MA MI MN MS MO MT NE NV NH NJ NM NY NC ND OH OK OR PA RI SC SD TN TX UT VT VA WA WV WI WY);
my %conus_states   = map { $_ => 1 } @conus_states;
...
my @keys_states   = keys %us_states;

if($regions eq 'states') {  # Need climdivs
    if (join($;,sort(@keys_pops)) ne join($;,sort(@keys_climdivs))) { die "Population data in $pops_file must be climate divisions - exiting"; }
}
else { # Need states
    if (join($;,sort(@keys_pops)) ne join($;,sort(@keys_states)))   { die "Population data in $pops_file must be states - exiting"; }
}

# --- Compute weights ---

my %sums;
if($regions eq 'states') { %sums = map { $_ => 0 } @us_states;           }
if($regions eq 'census') { %sums = map { $_ => 0 } keys %census_regions; }
if($regions eq 'conus')  { $sums{'conus'} = 0; }
if($regions eq 'us')     { $sums{'us'}    = 0; }

SUM: foreach my $region (keys %sums) {
    my $region_name = $region;
    if($regions eq 'census') { $region_name = $census_regions{$region}; }

    SUBREGION: foreach my $sub_region (keys %populations) {
        my $population = $populations{$sub_region};

        # Determine whether the sub-region falls within our region and add to the sum if so

        if($regions eq 'states') {
            my $state = substr($sub_region,0,2);
            if($state eq $region_name) { $sums{$region} += $population; }
        }
        elsif($regions eq 'census') {
            my @states_in_div = split(',',$region_name);
            my %states_in_div = map { $_ => 1 } @states_in_div;
            if(exists $states_in_div{$sub_region} and $sub_region ne 'AK' and $sub_region ne 'HI') { $sums{$region} += $population; }
        }
        elsif($regions eq 'conus') {
            if($sub_region ne 'AK' and $sub_region ne 'HI') { $sums{$region} += $population; }
        }
        else {
            { $sums{$region} += $population; }
        }
...
REGION: foreach my $region (keys %populations) {
    my $population = $populations{$region};

    if($regions eq 'states') {
        my $state        = substr($region,0,2);
        $popwts{$region} = $population/$sums{$state};
    }
    elsif($regions eq 'census') {
        my $sum;
        ...
        for(my $div=1; $div<10; $div++) {
            ...
            if(exists $states_in_div{$region} and $region ne 'AK' and $region ne 'HI') {
                $popwts{$region} = $population/$sums{$div};
            }
        ...
        if($region ne 'AK' and $region ne 'HI') { $popwts{$region} = $population/$sums{'conus'}; }
    }
    else {
        $popwts{$region} = $population/$sums{'us'};
    }
...
elsif($regions eq 'census' or $regions eq 'conus') { @regions = @conus_states;   }
else                                               { @regions = @us_states;      }
