...
package CPC::TemperatureData::Climdivs;

=pod

=head1 NAME

CPC::TemperatureData::Climdivs - Get temperature data from Climate Prediction Center climate divisions archives
...
sub get_max_data {
    ...
    my $date = shift;
    return &_get_data($date,$self->{TMAX_ARCHIVE},$self->{MISSING});
}
...
sub _get_data {
    ...
    foreach my $climdiv (@climdivs) {
        unless($data->{$climdiv}) { $data->{$climdiv} = $missing; }
    }
